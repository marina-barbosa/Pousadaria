...
describe "Inn API" do 
  context "GET api/v1/inns/:cnpj" do
    it "com sucesso, retorna detalhes" do
      ...
      # expect(inn_response["address_attributes.address"]).to eq("Rua X")
      # expect(inn_response["address_attributes.number"]).to eq("166")
      # expect(inn_response["address_attributes.city"]).to eq("Manaus")
      # expect(inn_response["address_attributes.state"]).to eq("AM")
      # expect(inn_response["address_attributes.postal_code"]).to eq("69067-080")
      # expect(inn_response["address_attributes.neighborhood"]).to eq("Centro")
